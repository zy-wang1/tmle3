...
target_p <- sapply(1:length(support_points), function(i) {
  data_out$f_d(support_points[i])
})
...
# support_points
# target_density %>% sum
# truth <- sum(target_density^2)/0.01
# truth
...
# haldensify not working for now
{
  ...
  hal_learner_density <- Lrnr_sl$new(learners = lrnr_dens_lib,
                                    metalearner = Lrnr_solnp_density$new())
  learner_list <- c(0, hal_learner_density)
...
likelihood_def <- Likelihood$new(factor_list)
tilde_likelihood <- likelihood_def$train(tmle_task)
...
{
  self <- tmle_params[[1]]
  support_points <- seq(-10, 10, by = 0.01)
...
updater_tilde <- tmle3_Update$new(convergence_type = "scaled_var",
                            constrain_step = T,
                            optim_delta_epsilon = F,
                            one_dimensional = F,
                            delta_epsilon = function(x) {
                            ...
                            ,
                            cvtmle=F)
targeted_tilde_likelihood <- Targeted_Likelihood$new(tilde_likelihood,
                                               submodel_type_by_node = "EIC" ,
                                               updater = updater_tilde)
tmle_params_tilde <- list(
  Param_ave_dens_tilde$new(observed_likelihood = targeted_tilde_likelihood, P_likelihood = initial_likelihood)
)
updater$tmle_params <- tmle_params_tilde


suppressMessages(
  fit_tilde <- fit_tmle3(tmle_task, targeted_tilde_likelihood, tmle_params_tilde, updater_tilde)
)
fit_tilde

tmle_params_tilde[[1]]$estimates()$IC %>% mean
sqrt(var(tmle_params_tilde[[1]]$estimates()$IC %>% as.vector)/tmle_task$nrow)/log(tmle_task$nrow)
targeted_tilde_likelihood$get_likelihood(tmle_task, "outcome", "full") %>% log %>% sum
...
targeted_likelihood_2 <- Targeted_Likelihood$new(initial_likelihood,
                                                 submodel_type_by_node = "EIC" ,
                                                 updater = updater_2)
tmle_param_2 <- Param_ave_dens_2$new(observed_likelihood = targeted_likelihood_2, targeted_likelihood_1 = targeted_likelihood, updater_1 = updater,
                                     tilde_likelihood =
                                       # targeted_likelihood
                                       targeted_tilde_likelihood
                                       )
