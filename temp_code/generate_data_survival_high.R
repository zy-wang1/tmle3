generate_Zheng_data_survival_high <- function(sample_size, tau, seed = NULL, setAM = NULL, if_LY_misspec = F, if_A_misspec = F, event_label = 0, tilt = 1) {
  ...
  if(is.null(setAM)) {
    ...
    while(t <= tau) {
      ...
      if (!if_A_misspec) {
        temp_A_C <- map_dbl(.x = expit(tilt + 1.5 - 0.8*L02 - 0.4*ifelse_vec(t > 1, temp_data[[t]]$L1, L01) + 0.5*ifelse_vec(t>1, temp_data[[t]]$A_E, 0)),
      ...
      if (!if_LY_misspec) {
        ...
        temp_Y <- map_dbl(.x = expit(- tilt + 0.2 + 1.5*L02 + temp_R + 0.2*temp_L - 0.3*temp_A_E - 0.3*temp_Z
                                     - 0.2*temp_A_E*temp_Z
        ...
        temp_Y <- map_dbl(.x = scale_bounded_relu(- tilt + 0.2 + 1.5*L02 + temp_R + 0.2*temp_L - 0.3*temp_A_E - 0.3*temp_Z
                                                  - 0.2*temp_A_E*temp_Z
