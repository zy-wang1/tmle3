# specify task name
identifier <- "new_resample"
# identifier <- "new_test"
# identifier <- "new_proj"
new_folder <- paste0("./parallel_", identifier)

# specify the template code
raw_code <- readLines("./temp_code/simulation_resample-20201111.R")
# raw_code <- readLines("./temp_code/simulation_new-20201109.R")
# raw_code <- readLines("./temp_code/simulation_proj-20201111.R")

# specify wd on cluster (each date should have a different folder) in the first line
raw_code <- c("setwd(\"~/20201116/tmle3\")",
              raw_code)
