...
n_mode <- 3
{
  ...
  bin_width <- 1e-0
  support_points <- seq(-5, 6, by = bin_width)
...
# true_pmf
true_pmf_transformed <- true_pmf
true_pmf_transformed$p[length(true_pmf_transformed$p) - 1] <- true_pmf_transformed$p[length(true_pmf_transformed$p) - 1] + last(true_pmf_transformed$p)
true_pmf_transformed <- true_pmf_transformed[-nrow(true_pmf_transformed), ]
truth <- sum(true_pmf_transformed$p^2)
sum(true_pmf$p^2)
truth
support_points <- seq(-5, 6, 1)  # supports are -5, -4, ..., 4, 5; 11 supports in total

# manually created bias
{
  B <- 300
  record <- matrix(0, 100, 3)
  for (i in 1:100) {
    ...
    record[i, 1] <- sum(predict(density_hal, new_A = seq(-5, 5, 1), new_W = 0)^2)
    record[i, 2] <- sum(((cut_interval(obs_x, n = n_bins, right = F) %>% table)/B)^2)
