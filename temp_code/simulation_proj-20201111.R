...
data_sim <- generate_Zheng_data(B = 5000, tau = 2, if_LY_misspec = if_misspec)
data_wide <- data.frame(data_sim)
...
tmle_params <- med_spec$make_params(tmle_task, initial_likelihood, static_likelihood = NULL)
suppressMessages(
  nontargeting2 <- tmle_params[[1]]$estimates(tmle_task)
)
temp_lmed3_nontargeting <- nontargeting2$psi
temp_IC <- nontargeting2$IC
var_D <- var(temp_IC)
n <- length(temp_IC)
se <- sqrt(var_D / n)
CI2_2 <- temp_lmed3_nontargeting + 1.96 * se
CI1_2 <- temp_lmed3_nontargeting - 1.96 * se
# end.time <- Sys.time()
# record_time[time_count] <- end.time - start.time
# time_count <- time_count + 1
...
suppressMessages(
  nontargeting_proj_2 <- tmle_params[[1]]$estimates(tmle_task)
)
temp_IC <- nontargeting_proj_2$IC
var_D <- var(temp_IC)
n <- length(temp_IC)
se <- sqrt(var_D / n)
CI2_proj_2 <- nontargeting_proj_2$psi + 1.96 * se
CI1_proj_2 <- nontargeting_proj_2$psi - 1.96 * se

capture.output(
suppressMessages(
  new_fit_2 <- fit_tmle3(tmle_task, tlik, tmle_params, updater)
)
)
# new_fit
# tlik$cache$tasks %>% lapply(function(x) x$data %>% dim)
new_psi_2 <- new_fit_2$summary$tmle_est
CI1_new_2 <- new_fit_2$summary$lower
CI2_new_2 <- new_fit_2$summary$upper
updater$step_number
...
data_sim <- generate_Zheng_data(B = 10000, tau = 2, if_LY_misspec = if_misspec)
data_wide <- data.frame(data_sim)
...
tmle_params <- med_spec$make_params(tmle_task, initial_likelihood, static_likelihood = NULL)
suppressMessages(
  nontargeting3 <- tmle_params[[1]]$estimates(tmle_task)
)
temp_lmed3_nontargeting <- nontargeting3$psi
temp_IC <- nontargeting3$IC
var_D <- var(temp_IC)
n <- length(temp_IC)
se <- sqrt(var_D / n)
CI2_3 <- temp_lmed3_nontargeting + 1.96 * se
CI1_3 <- temp_lmed3_nontargeting - 1.96 * se
# end.time <- Sys.time()
# record_time[time_count] <- end.time - start.time
# time_count <- time_count + 1
...
suppressMessages(
  nontargeting_proj_3 <- tmle_params[[1]]$estimates(tmle_task)
)
temp_IC <- nontargeting_proj_3$IC
var_D <- var(temp_IC)
n <- length(temp_IC)
se <- sqrt(var_D / n)
CI2_proj_3 <- nontargeting_proj_3$psi + 1.96 * se
CI1_proj_3 <- nontargeting_proj_3$psi - 1.96 * se

capture.output(
  suppressMessages(
    new_fit_3 <- fit_tmle3(tmle_task, tlik, tmle_params, updater)
  )
)
# new_fit
# tlik$cache$tasks %>% lapply(function(x) x$data %>% dim)
new_psi_3 <- new_fit_3$summary$tmle_est
CI1_new_3 <- new_fit_3$summary$lower
CI2_new_3 <- new_fit_3$summary$upper
updater$step_number
